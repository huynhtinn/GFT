...
def qdrant_rag_retrieval_node(state: SupportState) -> Dict[str, Any]:
    ...
    # Chuẩn hóa Confidence Score (%) cho Multilingual Semantic Search & Rerank
    # Cosine Similarity của paraphrase-multilingual-MiniLM-L12-v2 (hoặc Cohere Rerank v3)
    # cho các câu hỏi dài thường nằm trong khoảng 0.12 - 0.45 cho các kết quả khớp ngữ nghĩa cao.
    if best_score >= 0.35:
        confidence_score = min(99.0, round(90.0 + (best_score - 0.35) * 40.0, 1))
    elif best_score >= 0.12:
        # Khoảng 0.12 -> 0.35 quy đổi sang 55.0% -> 90.0% (Độ tin cậy RAG cao)
        confidence_score = round(55.0 + (best_score - 0.12) * (35.0 / 0.23), 1)
    elif best_score >= 0.05:
        # Khoảng 0.05 -> 0.12 quy đổi sang 25.0% -> 55.0% (Độ tin cậy trung bình)
        confidence_score = round(25.0 + (best_score - 0.05) * (30.0 / 0.07), 1)
    else:
        # Khoảng 0.00 -> 0.05 quy đổi sang 0.0% -> 25.0% (Độ tin cậy thấp)
        confidence_score = round(best_score * 500.0, 1)
